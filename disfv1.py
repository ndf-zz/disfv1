...
class fv1deparse(object):
    ...
    def __regmult__(self, inst, address):
        ...
        if inst['mnemonic'] == 'rdfx' and mult == 0:
            ...
            inst['comment'] = 'reg:0x{0:02x}'.format(reg)
        elif inst['mnemonic'] == 'maxx' and mult == 0 and reg == 0:
            inst['mnemonic'] = 'absa'
            inst['comment'] = 'maxx 0x0,0x0'
        else:
            inst['comment'] = 'reg:0x{0:02x} k:0x{1:04x}'.format(reg, mult)
    ...
    def __jam__(self, inst, address):
        ...
        lfo = inst['args'][0]
        inst['comment'] = 'lfo:0x{0:01x}'.format(lfo)
    ...
    def __delayop__(self, inst, address):
        ...
        offset = inst['args'][0]
        mult = inst['args'][1]
        inst['comment'] = 'del:0x{0:04x} k:0x{1:03x}'.format(offset, mult)
    ...
    def __rmpa__(self, inst, address):
        ...
        mult = inst['args'][0]
        inst['comment'] = 'k:0x{0:03x}'.format(mult)
    ...
    def __scaleoft__(self, inst, address):
        ...
        mult = inst['args'][0]
        offset = inst['args'][1]
        inst['comment'] = 'k:0x{0:04x} const:0x{1:03x}'.format(mult,offset)
    ...
    def __wldx__(self, inst, address):
        ...
        if inst['command'] & 0x40000000:
            ...
            inst['comment'] = 'lfo:0x{0:01x} f:0x{1:04x} a:0x{2:01x}'.format(
                 lfo, freq, amp)
        else:
            # WLDS
            ...
            inst['argstring'] = ','.join(['SIN'+str(lfo),
                  str(freq), str(amp) ])
            inst['comment'] = 'lfo:0x{0:01x} f:0x{1:03x} a:0x{2:04x}'.format(
                 lfo, freq, amp)
    ...
    def __skp__(self, inst, address):
        ...
        inst['comment'] = 'flags:0x{0:02x} offset:0x{1:02x}'.format(
                                flags, offset)
        flagv = []
        if flags == 0:
            flagv.append('0')
    ...
    def __fixinst__(self, inst, address):
        ...
        if inst['mnemonic'] == 'skp':
            ...
            self.dowarn('info: Unknown mnemonic: '
                         + repr(inst['mnemonic'])
                         + ' raw:0x{0:08x} at address:0x{1:02x}'.format(
...
def main():
    ...
    if args.p is not None:
        ...
        dowarn('info: Reading from program {0} at offset 0x{1:04X}'.format(
               args.p, oft))
